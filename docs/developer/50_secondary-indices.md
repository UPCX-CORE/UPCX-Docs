...
UPCX has the ability to sort tables by up to 16 indices. In the following section, we're going to add another index to the `addressbook` contract, so we can iterate through the records in a different way.
...
```shell
clupcx push action addressbook erase '["bob"]' -p bob@active
```
...
```shell
upcx-cpp --abigen addressbook.cpp -o addressbook.wasm
```
...
```shell
clupcx push action addressbook upsert '["alice", "alice", "liddell", 9, "123 drink me way", "wonderland", "amsterdam"]' -p alice@active
```

```shell
clupcx push action addressbook upsert '["bob", "bob", "is a guy", 49, "doesnt exist", "somewhere", "someplace"]' -p bob@active
```
...
```shell
clupcx get table addressbook addressbook people --upper 10 \
--key-type i64 \
--index 2
```
...
```shell
clupcx get table addressbook addressbook people --upper 50 --key-type i64 --index 2
```
...
private:
  ...
  using address_index = upcx::multi_index<"people"_n, person, indexed_by<"byage"_n, const_mem_fun<person, uint64_t, &person::get_secondary_1>>>;
