...
class [[upcx::contract("abcounter")]] abcounter : public upcx::contract {
  public:
  ...
  private:
    ...
    using count_index = upcx::multi_index<"counts"_n, counter>;
};
```

The first new concept in the code above is that we are explicitly restricting calls to the one action to a **specific account** in this contract using [require_auth](https://developers.upcx.io/manuals/upcx.cdt/latest/group__action/#function-require_auth) to the `addressbook` contract, as seen below.
...
Another new concept in the code above, is [action wrapper](https://developers.upcx.io/manuals/upcx.cdt/latest/structupcx_1_1action__wrapper). As shown below the first template parameter is the 'action' we are going to call and the second one should point to the action function
...
```shell
clupcx create account upcx abcounter YOUR_PUBLIC_KEY
```

## Step 3: Compile and Deploy

```shell
upcx-cpp abcounter.cpp -o abcounter.wasm
```

Finally, deploy the `abcounter` contract.

```shell
clupcx set contract abcounter CONTRACTS_DIR/abcounter
```
...
This time we use the [action wrapper](https://developers.upcx.io/manuals/upcx.cdt/latest/structupcx_1_1action__wrapper) instead of calling a function. To do that, we firstly initialised the count_action object defined earlier. The first parameter we pass is the callee contract name, in this case `abcounter`. The second parameter is the permission struct.

- For the permission, [get_self()](https://developers.upcx.io/manuals/upcx.cdt/latest/classupcx_1_1contract/#function-get_self) returns the current `addressbook` contract. The `active` permission of `addressbook` is used.
...
```shell
upcx-cpp -o addressbook.wasm addressbook.cpp -I ../abcounter/
```
...
```shell
clupcx push action addressbook upsert '["alice", "alice", "liddell", 19, "123 drink me way", "wonderland", "amsterdam"]' -p alice@active
```
...
```shell
clupcx get table abcounter abcounter counts --lower alice --limit 1
```
...
```shell
clupcx push action addressbook upsert '["alice", "alice", "liddell", 21,"1 there we go", "wonderland", "amsterdam"]' -p alice@active
```
...
```shell
clupcx push action abcounter count '["alice", "erase"]' -p alice@active
```

Checking the table in `abcounter` we'll see the following:

```shell
clupcx get table abcounter abcounter counts --lower alice
```
...
private:
  ...
  typedef upcx::multi_index<"people"_n, person, indexed_by<"byage"_n, const_mem_fun<person, uint64_t, &person::get_secondary_1>>> address_index;
};
```
