...
## Step 3: Write an Extended Standard Class and Include UPCX
...
Since this contract is an address book it should store some relevant details for each entry or _person_
...
Next, define a `primary_key` method. Every `multi_index` struct requires a _primary key_ method. Behind the scenes, this method is used according to the index specification of your `multi_index` instantiation. UPCX `multi_index` wraps [boost::multi_index](https://www.boost.org/doc/libs/1_59_0/libs/multi_index/doc/index.html)
...
Now that the data structure of the table has been defined with a `struct` we need to configure the table. The [upcx::multi_index](https://developers.upcx.io/manuals/upcx.cdt/latest/classupcx_1_1multi__index) constructor needs to be named and configured to use the struct we previously defined.

```cpp
using address_index = upcx::multi_index<"people"_n, person>;
```

With the above `multi_index` configuration there is a table named **people**, that

1. Uses the `_n` operator to define an upcx::name type and uses that to name the table. This table contains a number of different singular "persons", so name the table "people".
...
UPCX contracts extend the _contract_ class. Initialize our parent _contract_ class with the code name of the contract and the receiver. The important parameter here is the `code` parameter which is the account on the blockchain that the contract is being deployed to.
...
On a UPCX blockchain an account name is unique, therefore the `name` type is an ideal candidate as a **primary_key**. Behind the scenes, the [name](https://developers.upcx.io/manuals/upcx.cdt/latest/structupcx_1_1name) type is an `uint64_t` integer.
...
Earlier, it was mentioned that only the user has control over their own record, as this contract is opt-in. To do this, utilize the [require_auth](https://developers.upcx.io/manuals/upcx.cdt/latest/group__action/#function-require_auth) method provided by the `upcx.cdt`. This method accepts an `name` type argument and asserts that the account executing the transaction equals the provided value and has the proper permissions to do so.
...
2. The second parameter "scope" which ensures the uniqueness of the table in the scope of this contract. In this case, since we only have one table we can use the value from `get_first_receiver()`. The value returned from the _`get_first_receiver` function is the account name on which this contract is deployed to._

Note that scopes are used to logically separate tables within a multi-index (see the upcx.token contract multi-index for an example, which scopes the table on the token owner). Scopes were originally intended to separate table state in order to allow for parallel computation on the individual sub-tables. However, currently inter-blockchain communication has been prioritized over parallelism. Because of this, scopes are currently only used to logically separate the tables as in the case of upcx.token.
...
First, detect whether a particular user already exists in the table. To do this, use table's [find](https://developers.upcx.io/manuals/upcx.cdt/latest/classupcx_1_1multi__index#function-find) method by passing the `user` parameter. The find method will return an iterator. Use that iterator to test it against the [end](https://developers.upcx.io/manuals/upcx.cdt/latest/classupcx_1_1multi__index#function-end) method. The "end" method is an alias for "null".
...
Create a record in the table using the multi_index method [emplace](https://developers.upcx.io/manuals/upcx.cdt/latest/classupcx_1_1multi__index/#function-emplace). This method accepts two arguments, the "payer" of this record who pays the storage usage and a callback function.
...
Next, handle the modification, or update, case of the "upsert" function. Use the [modify](https://developers.upcx.io/manuals/upcx.cdt/latest/classupcx_1_1multi__index/#function-modify-12) method, passing a few arguments:
...
Instantiate the table. In `addressbook` each account has only one record. Set `iterator` with [find](https://developers.upcx.io/manuals/upcx.cdt/latest/classupcx_1_1multi__index/#function-find)
...
Finally, call the [erase](https://developers.upcx.io/manuals/upcx.cdt/latest/classupcx_1_1multi__index/#function-erase-12) method, to erase the iterator. Once the row is erased, the storage space will be free up for the original payer.
...
[upcx.cdt](https://developers.upcx.io/manuals/upcx.cdt/latest) includes an ABI Generator, but for it to work will require some declarations.
...
The `[[upcx.table]]` declaration will add the necessary descriptions to the ABI file.
...
```shell
upcx-cpp addressbook.cpp -o addressbook.wasm
```
...
```json
{
  "____comment": "This file was generated with upcx-abigen. DO NOT EDIT ",
...
```shell
clupcx create account upcx addressbook YOUR_PUBLIC_KEY -p upcx@active
```

Deploy the `addressbook` contract

```text
clupcx set contract addressbook CONTRACTS_DIR/addressbook -p addressbook@active
```

```shell
5f78f9aea400783342b41a989b1b4821ffca006cd76ead38ebdf97428559daa0  5152 bytes  727 us
#         upcx <= upcx::setcode               {"account":"addressbook","vmtype":0,"vmversion":0,"code":"0061736d010000000191011760077f7e7f7f7f7f7f...
...
```shell
clupcx push action addressbook upsert '["alice", "alice", "liddell", "123 drink me way", "wonderland", "amsterdam"]' -p alice@active
```
...
```text
clupcx push action addressbook upsert '["bob", "bob", "is a loser", "doesnt exist", "somewhere", "someplace"]' -p alice@active
```
...
If you are currently using 'clupcx push action' command, try to add the relevant authority using -p option.
Error Details:
...
```shell
clupcx get table addressbook addressbook people --lower alice --limit 1
```
...
```shell
clupcx push action addressbook erase '["alice"]' -p alice@active
```
